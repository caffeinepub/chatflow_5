...
export function InternetIdentityProvider({
  ...
  // Ref to signal that a re-init is needed (bumped by clear())
  const reinitRef = useRef(0);
  ...
  useEffect(() => {
    // reinitCount is read here so the effect re-runs after clear() bumps it
    void reinitCount;
    let cancelled = false;
    void (async () => {
      try {
